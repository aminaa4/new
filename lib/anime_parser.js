...
export const scrapeAnimeList = async ({ list = [], page = 1 }) => {
  try {
    ...
    $('div:nth-of-type(2) > div:nth-of-type(4) > main > section > div > div > div:nth-of-type(3) > div:first-child > div').each((i, el) => {
      list.push({
        animeTitle: $(el).find('h2').text().replace(/"/g, ""),
        animeId: $(el).find('a').attr('href').replace("https://anime3rb.com/titles/", ""),
      });
    });
    return list;
  } catch (err) {
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animeImage = $('body > div.flex.flex-grow.h-full.bg-gray-100.dark\:bg-dark-800.relative > div.flex.justify-between.flex-col.flex-grow.overflow-hidden > main > div > section:nth-child(1) > div > div > div > div.w-full.lg\:max-w-\[18rem\].md\:w-72.flex-grow.flex.flex-col.gap-3 > img').attr('src');
    const type = $('div.anime_info_body_bg > p:nth-child(4) > a').text();
    const desc = $('body > div.flex.flex-grow.h-full.bg-gray-100.dark\:bg-dark-800.relative > div.flex.justify-between.flex-col.flex-grow.overflow-hidden > main > div > section:nth-child(1) > div > div > div > div.w-\[500px\].flex-grow.flex.flex-col.gap-4 > p')
      .text()
      .replace('Plot Summary: ', '');
    const releasedDate = $('body > div.flex.flex-grow.h-full.bg-gray-100.dark\:bg-dark-800.relative > div.flex.justify-between.flex-col.flex-grow.overflow-hidden > main > div > section:nth-child(1) > div > div > div > div.w-\[500px\].flex-grow.flex.flex-col.gap-4 > div.block.lg\:hidden > div > table > tbody > tr:nth-child(2) > td:nth-child(2)')
      .text()
      .replace('Released: ', '');
    const status = $('body > div.flex.flex-grow.h-full.bg-gray-100.dark\:bg-dark-800.relative > div.flex.justify-between.flex-col.flex-grow.overflow-hidden > main > div > section:nth-child(1) > div > div > div > div.w-\[500px\].flex-grow.flex.flex-col.gap-4 > div.block.lg\:hidden > div > table > tbody > tr:nth-child(1) > td:nth-child(2)').text();
    const otherName = $('body > div.flex.flex-grow.h-full.bg-gray-100.dark\:bg-dark-800.relative > div.flex.justify-between.flex-col.flex-grow.overflow-hidden > main > div > section:nth-child(1) > div > div > div > div.w-\[500px\].flex-grow.flex.flex-col.gap-4 > div.flex.flex-col.gap-3.rounded-lg > div')
      .text()
      .replace('Other name: ', '')
      .replace(/;/g, ',');
