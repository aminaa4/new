...
export const scrapeGenre = async ({ list = [], genre, page = 1 }) => {
  try {
    genre = genre.trim().replace(/ /g, '-').toLowerCase();
      ...
      list.push({
        animeTitle: $(el).find('a:first-child > h2').text().replace(/"/g, ""),
...
export const scrapeStudio = async ({ studio, page = 1 }) => {
  try {

      let series = [];
    
      const studioPage = await axios.get(`https://anime3rb.com/c/studio/${studio}?page=${page}`);
      

      const $ = cheerio.load(studioPage.data);
...
export const scrapeAnimeTv = async ({ list = [], page = 1 }) => {
  try {
    const AnimeList = await axios.get(`${BASE_URL}/titles/list/tv?page=${page}`);
    const $ = cheerio.load(AnimeList.data);
...
export const scrapeWatchAnime = async ({ id, episodeId }) => {
  try {
    ...
    $('html > body > div:nth-of-type(2) > div:nth-of-type(4) > main > div > section:first-of-type > div > div > div:first-of-type > div:nth-of-type(2) > div:nth-of-type(4) > div:nth-of-type(2) > div:first-of-type > div > div').each((i, el) => {
      down.push({
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    // Cache main elements
    const mainSection = $('body > div:nth-of-type(2) > div:nth-of-type(4) > main');
    
    const animeTitle = mainSection.find('section:nth-of-type(1) > div > div > div > div:nth-of-type(2) > div:first-child > h1 > span:first-child').text();
    const animeImage = mainSection.find('section:nth-of-type(1) > div > div > div > div:nth-of-type(1) > img').attr('src');
    const type = mainSection.find('section:first-of-type > div > div > div > div:nth-of-type(2) > div:nth-of-type(6) > div > table > tbody > tr:nth-of-type(2) > td:nth-of-type(2)').text();
    const desc = mainSection.find('section:first-of-type > div > div > div > div:nth-of-type(2) > div:nth-of-type(4)').text();
    const status = mainSection.find('section:first-of-type > div > div > div > div:nth-of-type(2) > div:nth-of-type(6) > div > table > tbody > tr:first-of-type > td:nth-of-type(2)').text();
    
    mainSection.find('section:nth-of-type(1) > div > div > div > div:nth-of-type(2) > div:nth-of-type(2) > a').each((_, elem) => {
      genres.push($(elem).text().trim());
    });

    const ep_end = mainSection.find('section:nth-of-type(1) > div > div > div > div:nth-of-type(2) > div:nth-of-type(3) > div:nth-of-type(2) > div:nth-of-type(2) > p:nth-of-type(2)').text();

    const studioLink = mainSection.find('section:first-of-type > div > div > div > div:first-of-type > div:nth-of-type(2) > div > table > tbody > tr:nth-of-type(3) > td:nth-of-type(2) > a:first-of-type');
    
    if (studioLink.length) {
      const href = studioLink.attr('href');
      if (href?.includes("studio")) {
        studioText = studioLink.text().trim().replace(/\n/g, '');
        studioUrl = href.replace('https://anime3rb.com/c/studio/', '');
      }
    }

    const rating = mainSection.find('div.border:nth-child(1) > div:nth-child(2) > p:nth-child(2)').text();
    const age_rate = mainSection.find('div.dark\\:border-dark-700:nth-child(3) > div:nth-child(2) > p:nth-child(2)').text();
    const malId = mainSection.find('section:first-of-type > div > div > div > div:nth-of-type(2) > div:nth-of-type(7) > div > a:first-of-type').attr('href');

    mainSection.find('section:nth-of-type(2) > div > div > div:nth-of-type(2) > ul > li > div > a:first-child').each((_, el) => {
      animeRelated.push({
    ...
    mainSection.find('section:first-of-type > div > div > div > div:nth-of-type(2) > div:nth-of-type(5) > div > h2').each((_, el) => {
      otherNames.push({
        name: $(el).text().replace(/;/g, ','),
      });
    });

    const primarySelector = 'section:nth-of-type(4) > div > div > div:nth-of-type(2) > ul > li > div';
    const fallbackSelector = 'section:nth-of-type(3) > div > div > div:nth-of-type(2) > ul > li > div';
    const selectedElements = mainSection.find(primarySelector).length ? mainSection.find(primarySelector) : mainSection.find(fallbackSelector);
    ...
    mainSection.find('a[href^="https://anime3rb.com/episode/"]').each((_, el) => {
      epList.push({
    ...
    mainSection.find('iframe[data-src]').each((_, el) => {
