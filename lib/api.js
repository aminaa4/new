...
app.get('/animeListTv', async (req, res) => {
  try {
    const page = req.query.page;

    const data = await scrapeAnimeTv({ page: page });
...
app.get('/animeListOva', async (req, res) => {
  try {
    const page = req.query.page;

    const data = await scrapeAnimeOva({ page: page });
...
app.get('/animeListOna', async (req, res) => {
  try {
    const page = req.query.page;

    const data = await scrapeAnimeOna({ page: page });
...
app.get('/studio/:studio', async (req, res) => {
  try {
    const studio = req.params.studio;
    const page = req.query.page;

    const data = await scrapeStudio({ studio: studio, page: page });
...
// anime 3rb recent anime episodes
app.get('/recent-release', async (req, res) => {
  try {
    const page = req.query.page;

    const data = await scrapeRecentRelease({ page: page });
...
// anime 3rb recent anime added
app.get('/getRecentlyAdded', async (req, res) => {
  try {
    const page = req.query.page;
    const data = await scrapeRecentlyAdded();
...
app.get('/top-airing', async (req, res) => {
  try {
    ...
    const data = await scrapeTopAiringAnime();
...
app.get('/getEpisode/:id/:episodeId', async (req, res) => {
  
  try {
    const id = req.params.id; // Capture the anime ID from the request parameters
    const episodeId = req.params.episodeId; // Capture the episode ID from the request parameters

    // Call your scraping function with both IDs
    const data = await scrapeWatchAnime({ id: id, episodeId: episodeId });

    res.status(200).json(data); // Send back the scraped data as JSON
  } catch (err) {
